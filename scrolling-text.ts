...
export function setupScrollingText({
    ...
    const backBtnElement = menuElement.querySelector("button:first-child") as HTMLButtonElement | null;
    const nextBtnElement = menuElement.querySelector("button:last-child") as HTMLButtonElement | null;
    ...
    const setPageValueSideBarContent = (value: string) => pagesElement.setAttribute("page", value);
    ...
    const arrayLiElementButtonToggle: Array<HTMLLIElement> = listPageName.map((name, index) => {
    ...
    function updatePage() {
        ...
        arrayLiElementButtonToggle.forEach(li => {
            if (li.firstElementChild !== null) {
                const buttonToggle = li.firstElementChild as HTMLButtonElement;
        ...
        listPageId.forEach(page => {
            const pageElement = pagesElement.querySelector("#" + page) as HTMLElement | null;
    ...
    if (nextBtnElement !== null) nextBtnElement.onclick = () => changePage("next");
    if (backBtnElement !== null) backBtnElement.onclick = () => changePage("back");
